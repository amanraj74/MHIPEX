...
In this work we present MHIPEX (\textbf{M}ultilingual \textbf{H}istorical \textbf{I}nformation \textbf{P}erson-place \textbf{EX}traction), our participating system. We pursue five research questions:
...
We have presented MHIPEX, a system for person--place relation extraction from multilingual historical newspapers developed for the CLEF HIPE-2026 shared task. Our five research questions are answered as follows: multilingual transformers effectively model person--place relations (RQ1); input enrichment with date and language tokens provides consistent gains (RQ2); combining hmBERT and XLM-R yields complementary improvements (RQ3); the \textsc{probable} class remains the primary bottleneck (RQ4); and our proposed Relation-Specific Language-Adaptive Ensemble (RLAE) outperforms the fixed-weight ensemble by +0.011~MR, confirming that adaptive fusion is beneficial (RQ5). MHIPEX-RLAE achieves a macro-recall of 0.5767 (95\% CI: $[0.548, 0.604]$) on the sandbox development data---a 35.1\% relative improvement over mBERT---and domain transfer to newspaper v1.0 yields an MR of 0.588.
