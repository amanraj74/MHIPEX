...
\begin{table}[t]
\centering
\caption{Learned RLAE parameters per language. $\beta^{(r)}_\ell$ denotes the hmBERT weight for relation $r$ in language $\ell$.}
\label{tab:rlae}
\begin{tabular}{lcccc}
\toprule
...
We have presented MHIPEX, a general framework for person--place relation extraction from multilingual historical newspapers. Evaluated within the rigorous CLEF HIPE-2026 shared task setting, our findings demonstrate that multilingual transformers effectively model these complex relations (RQ1) and that input enrichment with date and language tokens provides consistent gains (RQ2). Furthermore, combining hmBERT and XLM-R yields complementary improvements (RQ3), though the ambiguous \textsc{probable} class remains the primary bottleneck (RQ4). Most significantly, our proposed Relation-Specific Language-Adaptive Ensemble (RLAE) outperforms the fixed-weight ensemble by +0.011~MR, confirming that conditional adaptive fusion is highly beneficial (RQ5). Overall, MHIPEX-RLAE achieves a macro-recall of 0.5767 (95\% CI: $[0.548, 0.604]$) on the sandbox development data---a 35.1\% relative improvement over mBERT---and proves its broad applicability with a domain transfer MR of 0.588 on newspaper v1.0.
