...
While the individual components of MHIPEX---historically pre-trained transformers, class-weighted cross-entropy, and basic ensembling---are established techniques, the genuine methodological novelty of our framework lies in the RLAE architecture. By decomposing the ensemble conditionally on both relation type and source language, RLAE achieves the benefits of a Mixture-of-Experts routing network without requiring end-to-end retraining.
...
\begin{table}[t]
\centering
\caption{Comparison of loss functions on the hmBERT encoder. Weighted Cross-Entropy yields the strongest overall macro-recall.}
...
These facts were serialized into text strings and appended to the input using special tokens (e.g., \texttt{<WIKI>}, \texttt{<GEONAMES>}, \texttt{<GETTY>}). 

\begin{table}[t]
\centering
\caption{Impact of Knowledge Graph augmentation across different KG sources (hmBERT encoder).}
\label{tab:kg}
\begin{tabular}{lccc}
\toprule
...
E1: Text + Wikidata & \textbf{0.565} & 0.476 & 0.654 \\
E2: Text + GeoNames & 0.559 & \textbf{0.479} & 0.639 \\
E3: Text + Getty TGN & 0.550 & 0.453 & 0.647 \\
\bottomrule
\end{tabular}
\end{table}
...
We have presented MHIPEX, a general framework for person--place relation extraction from multilingual historical newspapers. Evaluated within the rigorous CLEF HIPE-2026 shared task setting, our findings suggest that multilingual transformers effectively model these complex relations (RQ1) and that input enrichment with date and language tokens provides consistent gains (RQ2). Furthermore, combining hmBERT and XLM-R yields complementary improvements (RQ3), though the ambiguous \textsc{probable} class remains the primary bottleneck (RQ4). Most significantly, our proposed Relation-Specific Language-Adaptive Ensemble (RLAE) outperforms the fixed-weight ensemble by +0.011~MR, confirming that conditional adaptive fusion is highly beneficial (RQ5). Overall, MHIPEX-RLAE achieves a macro-recall of 0.5767 (95\% CI: $[0.548, 0.604]$) on the sandbox development data---a 35.1\% relative improvement over mBERT---and suggests its broad applicability with a domain transfer MR of 0.588 on newspaper v1.0.

The RLAE analysis reveals a key insight: while spatial relations (\texttt{at}) consistently prefer hmBERT across all languages, the optimal encoder for temporal relations (\texttt{isAt}) varies by language---English favors hmBERT ($\beta=0.70$) while French and German favor XLM-R ($\beta \leq 0.35$). This pattern was predicted by our Knowledge Graph experiment, which demonstrated that static gazetteer facts (GeoNames) improve spatial reasoning (+1.2\,pp) but degrade temporal reasoning ($-$2.0\,pp), and it generalizes the spatial--temporal dichotomy into a concrete algorithmic contribution. Taken together, these findings validate the hypothesis raised in Section~\ref{sec:related} that XLM-R's broader world knowledge contributes complementary reasoning capabilities for the knowledge-intensive \texttt{at} relation, as confirmed by both the GeoNames augmentation gain (+1.2\,pp in \texttt{at} recall) and the RLAE weights (French and German \texttt{isAt} routing 65--70\% of weight to XLM-R).
