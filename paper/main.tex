...
Severe class imbalance is endemic to information extraction tasks. Lin et al.~\cite{lin2017} introduced focal loss to down-weight well-classified examples. In our task, where the minority class (\texttt{at}=\textsc{true}) comprises only 12\% of examples and \texttt{isAt}=\textsc{true} only 8.4\%, addressing imbalance is not optional but essential. We adopt class-weighted cross-entropy loss functions calibrated to the inverse frequency of each label. As detailed in our methodological ablations (Section~\ref{sec:loss_ablation}), this class-weighted formulation empirically outperforms both standard unweighted cross-entropy and focal loss ($\gamma=2$) on our dataset.
...
\begin{table}[t]
\centering
\caption{Main results on the HIPE-2026 sandbox development set. MR denotes macro-recall (official metric). $\dagger$ indicates threshold calibration applied. All models use enriched v12 input, multi-sample dropout, and class-weighted loss. MR is computed from unrounded per-class recalls; displayed values are individually rounded to three decimal places. Unrounded arithmetic: for hmBERT$^{\dagger}$, MR\_at=0.4502 and MR\_isAt=0.6558 yields MR=0.5530.}
\label{tab:main}
\begin{tabular}{llccc}
\toprule
